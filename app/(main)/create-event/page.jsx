...
const CreateEvent = () => {
  ...
  //check if user has a pro plan or not....
  const { has } = useAuth();
  const { data: currentUser } = useConvexQuery(api.users.getCurrentUser);
  const hasPro = currentUser?.plan === "pro" || has?.({ plan: "pro" });

  const [ticketTiers, setTicketTiers] = useState([
    { id: "regular", name: "Regular", price: 0, capacity: 50 },
  ]);
  ...
  const handleAddTicketTier = () => {
    if (!hasPro) {
      setUpgradeReason("tickets");
      setShowUpgradeModal(true);
      return;
    }
    setTicketTiers([
      ...ticketTiers,
      { id: `tier-${Date.now()}`, name: "VIP", price: 299, capacity: 50 },
    ]);
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if(!hasPro && (currentUser?.freeEventsCreated >= 1 || currentUser?.plan !== "pro")){
        setUpgradeReason("limit");
        setShowUpgradeModal(true);
        return;
      }

      // Check capacity limit for free users (50 max)
      if (!hasPro && data.capacity > 50) {
        setUpgradeReason("attendee");
        setShowUpgradeModal(true);
        toast.error("Free plan supports up to 50 attendees per event. Upgrade to Pro for up to 1,000 attendees!");
        return;
      }
      ...
      await createEvent({
        ...
        capacity: !hasPro ? Math.min(data.capacity, 50) : Math.min(data.capacity, 1000),
        ticketType: data.ticketType,
        ticketPrice: data.ticketPrice || undefined,
        tickets: hasPro && ticketTiers.length > 1 ? ticketTiers : undefined,
        coverImage: data.coverImage || undefined,
        themeColor: !hasPro ? "#1e3a8a" : data.themeColor,
      });
